...
function normalizeInstagram(input?: string) {
  ...
  if (/^https?:\/\//i.test(s)) {
    try {
      ...
      // Ignore non-profile routes (post/reel/story etc.)
...
function normalizeFacebook(input?: string) {
  ...
  if (/^https?:\/\//i.test(s)) {
    try {
      ...
      // canonicalize host to facebook.com
...
export default function RegistrationDesktop() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // normalize socials one last time (server should also validate, but this keeps DB clean)
    const ig = normalizeInstagram(form.instagram);
    const fb = normalizeFacebook(form.facebook);

    // if user filled something but it is invalid -> show message
    if (form.instagram.trim() && !ig) {
      setMsg("Instagram: please paste your profile (@username or profile link)");
      return;
    }
    if (form.facebook.trim() && !fb) {
      setMsg("Facebook: please paste your profile link or username");
      return;
    }
            ...
            <aside className="h-80 col-span-4">
              <div className="rounded-2xl border border-gray-200 p-5 h-full flex flex-col items-center justify-center gap-4">
                <PictureUploader
                  pic_url={form.pic_url}
                  pic_public_id={form.pic_public_id}
                  onChange={({ url, public_id }) => setForm({ ...form, pic_url: url, pic_public_id: public_id })}
                />
                <p className="text-sm text-gray-500 text-center">Upload a clear headshot. JPG/PNG, ~2–5 MB.</p>
              </div>
            </aside>
